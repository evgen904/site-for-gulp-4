jQuery(document).ready(function ($) {
    ...
    $('#home-fullpage').fullpage({
        ...
        onLeave: function (origin, destination, direction) {
            ...
            /*let menuLine = $(`#header .indicator`);
            let anchor = $(destination.item).data('anchor');
            let activeItem = $(`#header li[data-menuanchor="${anchor}"]`);
            if (activeItem.hasClass('no-indicator')) {
                menuLine.css({
                    width: '0px',
                });
            } else {
                let itemW = activeItem.outerWidth();
                let itemPos = activeItem.offset().left - $(`#header nav`).offset().left;
                menuLine.css({
                    width: (itemW + 20) + 'px',
                    left: (itemPos - 10) + 'px'
                });
